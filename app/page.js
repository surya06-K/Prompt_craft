  ...
  async function generate() {
    ...
    try {
      const res = await fetch('/api/generate', {
        ...
        body: JSON.stringify({ goal, role, format, tone, audience, extra, provider }),
      })
        ...
        <div style={paneLeft}>
          ...
          <div style={tipsBox}>
            ...
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '7px' }}>
              {[
                'Be specific — mention product, audience, or situation',
                'Pick a role that matches your use case closely',
                'Use constraints to control length and language',
                'Combine format + tone for precise output control',
              ].map((t, i) => (
